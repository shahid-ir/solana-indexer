...
class CPIEventsDebugger {
    coder = new anchor_1.BorshCoder(idl_1.DYNAMIC_BONDING_CURVE_IDL);
            ...
            if (base64) {
                try {
                    ...
                    if (decoded.length >= 8) {
                        ...
                        console.log(`   🎯 Expected EvtSwap: ${idl_1.SWAP_EVENT_DISCRIMINATOR.toString('hex')}`);
                        console.log(`   🎯 Matches EvtSwap: ${first8Bytes.equals(idl_1.SWAP_EVENT_DISCRIMINATOR)}`);
                        if (first8Bytes.equals(idl_1.SWAP_EVENT_DISCRIMINATOR)) {
                            console.log('   🎯 FOUND EVT_SWAP EVENT!');
                            eventCount++;
                            try {
                                const event = this.coder.events.decode(base64);
                                if (event) {
                                    console.log('   ✅ Successfully decoded event:', event.name);
    ...
    async runAllTests() {
        ...
        console.log(`🔍 EvtSwap Discriminator: ${idl_1.SWAP_EVENT_DISCRIMINATOR.toString('hex')}`);
...
//# sourceMappingURL=debug-cpi-events.js.map